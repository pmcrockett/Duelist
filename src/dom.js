...
export function createCard(_task) {
  ...
  if (_task.dueDate) {
    createAppend(
      ...
      hDiv,
      `${dateFormat(_task.dueDate, "dddd, mmmm dS, yyyy")}`
    );
  }
  ...
  if (_task.dueTime) {
    createAppend(
      ...
      hDiv,
      `${dateFormat(_task.dueTime, "h:MM TT")}`
    );
  }
...
function createSvg(_path, _title, _useBg, _pathClass) {
  ...
  if (_useBg) {
    ...
    bgPath.setAttribute(
      "d",
      "M12,4A10,10 0 0,0 4,12A10,10 0 0,0 12,20A10,10 0 0,0 20,12A10,10 0 0,0 12,4Z"
    );
